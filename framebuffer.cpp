...
int File = open("/dev/fb1", O_RDWR);  //MiWa 20191109 here we need fb1
...
FrameBuffer::FrameBuffer(int pFile,uint8_t* pFrameBuffer,struct fb_fix_screeninfo pFixInfo,struct fb_var_screeninfo pScreenInfo,bool pVerbose):
	...
	mXOffset(pScreenInfo.xoffset),
	mYOffset(pScreenInfo.yoffset)
{
}
...
{
//std::cout << "DrawCahr: pY=" << pY << std::endl;
	//const uint8_t* d = font8x13 + pChar * 13;
const uint8_t* d = fontdata_8x8 + pChar * MYFONTSIZE; //MIWA 20191110

	if( mPixelSize == 1 )
	{
		for(int y = 0 ; y < MYFONTSIZE ; y++ , d++) //MIWA 20191110
		{
			int x = pX;
	...
	{
		for(int y = 0 ; y < MYFONTSIZE ; y++ , d++ , pY += mPixelSize ) //MIWA 20191110
		{
			int x = pX;
			const uint8_t bits = *d;
			if( bits != 0 )
				...
				for( int bit = 7 ; bit > -1 ; bit-- , x+=mPixelSize )
				{
					if( bits&(1<<bit) )
					{
						for(int i = 0 ; i < mPixelSize ; i++ )
							pDest->DrawLineH(x,pY+i,x+mPixelSize,pRed,pGreen,pBlue);
					}
				}
			}
		}
...
{
	//std::cout << "Printf: pY=" << pY << std::endl;
	char buf[1024];
	va_list args;
	va_start(args, pFmt);
